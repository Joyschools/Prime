...
def communication_send():
    ...
    message_id=execute('INSERT INTO communication_messages(sender_user_id,recipient_user_id,body) VALUES(?,?,?)',(current_user()['id'],rid,body))
    notify_user(rid, f'New message from {current_user()["full_name"]}', body[:180], url_for('communication_center'), 'High')
    audit(current_user()['id'],current_user()['full_name'],'Internal Message',f'Message sent to {target["full_name"]}.')
    flash('Message sent.','success'); return redirect(url_for('communication_center'))
...
@login_required
@role_required("Admin","ICT","Librarian","Teacher")
def add_library_item():
    ...
    return redirect(url_for('teacher_dashboard') if current_user()['role']=='Teacher' else (url_for("ict_dashboard") if current_user()["role"]=="ICT" else url_for("librarian_dashboard") if current_user()["role"]=="Librarian" else url_for("admin_dashboard"))+"#library")
